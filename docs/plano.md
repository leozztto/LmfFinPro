...
- Exportação de transações em CSV — ✅ **feito** (adiantado, sétimo tipo de relatório na tela "Relatórios") — extrato bruto de todas as transações do usuário (em todas as contas) num mês, incluindo transferências, pronto para abrir no Excel/Google Sheets. `ReportController` (`/api/reports/transaction-export?referenceMonth=`), `ReportApplicationService.generateTransactionExport` resolve nome de conta/categoria/cliente e `TransactionCsvExporter` (novo port `TransactionExportPort`) monta o CSV na mão — mesma filosofia do `CsvTransactionParser`, mas na direção inversa (escrita) — com BOM UTF-8 para o Excel reconhecer acentuação
- Multi-moeda
...
- Exportação de relatórios (PDF/Excel) — ✅ **feito**: todos os sete tipos de relatório em PDF (ver itens acima) mais exportação de transações em CSV (abre no Excel); não existe geração de .xlsx binário — decisão consciente para não adicionar uma lib pesada (Apache POI) só para isso
