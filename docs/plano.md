...
- Lançamentos recorrentes — ✅ **feito** (não estava no plano original). `RecurringTransaction` é um modelo (conta, categoria/cliente opcionais, valor, tipo, frequência semanal/mensal/anual, data inicial e data final opcional) que vira uma `Transaction` real (`origin = RECURRING`, `recurring_transaction_id`) a cada ocorrência vencida. O progresso é guardado como contador (`generated_occurrences`), e a próxima data é sempre `startDate + N períodos`, então uma recorrência no dia 31 não fica presa no dia 28 depois de fevereiro. As ocorrências vencidas são lançadas na criação (data inicial no passado gera o retroativo), por um `@Scheduled` diário às 00:05 (`America/Sao_Paulo`, cron configurável em `finpro.recurring.cron`) e na subida da aplicação, que recupera dias perdidos com o servidor fora do ar. Cada recorrência roda na sua própria transação de banco. Conta, tipo, frequência e data inicial são imutáveis; ao reativar uma recorrência pausada, as datas da pausa são puladas; excluir a recorrência mantém as transações lançadas (FK `ON DELETE SET NULL`); conta com recorrência não pode ser excluída. `Clock` injetável (bean em `SchedulingConfig`) define o "hoje" no fuso do usuário. Frontend: tela "Recorrentes" (`/recorrentes`) e etiqueta "Recorrente" na lista de transações. Migration V13. A projeção de fluxo de caixa (`DashboardAggregator.cashFlowProjection`) usa as recorrências. Cada mês futuro soma a parte variável (lançamentos avulsos já cadastrados ou, sem eles, a média móvel dos últimos 3 meses) às ocorrências ainda não lançadas das recorrências ativas, nas datas exatas. As transações já lançadas por uma recorrência ativa saem da média, para não contar duas vezes, e as ocorrências pendentes até o fim do mês atual ajustam o saldo de partida
- Situação paga/pendente das transações — ✅ **feito** (não estava no plano original). `TransactionStatus` (`PAID`/`PENDING`, migration V14; lançamentos manuais antigos com data futura migrados para pendente). O **saldo atual** das contas (`sumPaidAmountByAccountIdAndType`, que também valida saldo nas transferências) considera só as pagas; o Dashboard ganhou `pendingIncome`, `pendingExpense` e `projectedBalance` (saldo atual + a receber − a pagar). Relatórios, orçamentos, imposto, fluxo mensal e projeção seguem por competência (pagas e pendentes). Padrões: manual sem situação informada usa `TransactionStatus.defaultFor` (data futura = pendente, com o "hoje" do `Clock`); importadas e transferências sempre pagas; ocorrências recorrentes nascem pendentes. `PATCH /api/transactions/{id}/status` alterna a situação (transferência não pode ficar pendente → 400). Frontend: campo "Situação" no formulário (Automática/Paga/Pendente), filtro por situação, etiqueta "A receber"/"A pagar" e botão de alternar no card, cards "A receber", "A pagar" e "Saldo previsto" no Dashboard
- Alertas e lembretes por e-mail — ✅ **feito** (não estava no plano original). Um resumo diário por usuário (`AlertScheduler`, `@Scheduled` às 08:00 `America/Sao_Paulo`, cron em `finpro.alerts.cron`; não roda na subida da aplicação), enviado só quando há algo novo: **contas a vencer** (despesas pendentes com data entre hoje e hoje + N dias), **orçamentos do mês** que passaram de 80% e de 100% (mesmo gasto de `BudgetApplicationService.calculateSpent`) e **DAS** para MEI/Simples Nacional (sempre no dia 20 do mês seguinte à competência, `DasSchedule`, com o valor da `TaxEstimate` da competência quando existir). Cada aviso vai uma vez só: a tabela `sent_alerts` (UNIQUE por usuário + tipo + chave) guarda o que já foi avisado, e só é gravada depois do envio. Ao passar direto para 100%, o aviso de 80% também é marcado. Envio pela porta `AlertMailerPort`, no mesmo esquema do reset de senha (SMTP com `spring.mail.host`, senão log). Preferências por usuário em `notification_preferences` (liga/desliga por tipo + antecedência de 0 a 15 dias, padrão 3, tudo ligado se não houver linha), expostas em `GET/PUT /api/profile/notifications` e na aba "Notificações" de Configurações. Migration V15. Fluxo em [`fluxo-alertas.md`](./tecnica/fluxo-alertas.md)

### Fase 3

- Recibo/cobrança em PDF por cliente — ✅ **feito** (adiantado em relação ao plano original) — tela "Relatórios" (`/relatorios`, estruturada para múltiplos tipos de relatório) gera um recibo em PDF somando as receitas lançadas para um cliente num mês. Backend: `ReportController` (`/api/reports/client-receipt`), `ReportApplicationService` busca cliente/emissor/transações e `OpenPdfReceiptGenerator` (lib OpenPDF, sem template engine, mesma filosofia "monta na mão" do `CsvTransactionParser`) monta o PDF — primeira resposta binária do backend. Frontend: `httpClient.getBlob` (primeiro download de arquivo do projeto) + `downloadBlob` (`URL.createObjectURL`)
- Extrato de conta em PDF — ✅ **feito** (adiantado, segundo tipo de relatório na tela "Relatórios") — saldo de abertura do período (saldo inicial + tudo antes dele), todas as movimentações (receitas e despesas) do mês com saldo corrente linha a linha, e saldo de fechamento. `ReportController` (`/api/reports/account-statement`) e `ReportApplicationService.generateAccountStatement` reaproveitam `TransactionRepositoryPort.findAllByAccountIds` já existente, dividindo as transações em memória por data (sem query nova)
- Demonstrativo anual de receita por cliente em PDF — ✅ **feito** (adiantado, terceiro tipo de relatório na tela "Relatórios") — total recebido do cliente em cada um dos 12 meses de um ano (mesmo os zerados) + total anual, útil para declaração de IR ou para enviar ao próprio cliente. `ReportController` (`/api/reports/client-annual-statement?clientId=&year=`) e `ReportApplicationService.generateClientAnnualStatement` reaproveitam `findAllByClientIdAndTypeAndDateBetween` já existente, passando o intervalo do ano inteiro em vez de um mês
- Relatório de despesas por categoria em PDF — ✅ **feito** (adiantado, quarto tipo de relatório na tela "Relatórios") — soma as despesas do usuário (em todas as contas) por categoria num mês, ordenadas da maior para a menor, útil para organizar dedutíveis. `ReportController` (`/api/reports/category-expenses?referenceMonth=`) e `ReportApplicationService.generateCategoryExpenseReport` cruzam `AccountRepositoryPort`/`TransactionRepositoryPort`/`CategoryRepositoryPort.findAllVisibleToUser` já existentes; transações sem categoria entram como "Sem categoria" e categorias excluídas depois de usadas como "Categoria removida"
- Resultado do período (DRE simplificado) em PDF — ✅ **feito** (adiantado, quinto tipo de relatório na tela "Relatórios") — receita, despesa e resultado (receita − despesa) do usuário consolidados por mês, trimestre ou ano, à escolha de quem gera o relatório, com totais gerais ao final. `ReportController` (`/api/reports/income-statement?year=&granularity=`) e `ReportApplicationService.generateIncomeStatement` reaproveitam `AccountRepositoryPort`/`TransactionRepositoryPort.findAllByAccountIds` já existentes, excluindo transferências entre contas próprias do mesmo jeito que o dashboard (`transferId() == null`)
- Orçamento vs. realizado em PDF — ✅ **feito** (adiantado, sexto tipo de relatório na tela "Relatórios") — reaproveita os `Budget` já cadastrados no mês, comparando o limite definido com o gasto real na categoria (mesmo cálculo de `BudgetApplicationService.calculateSpent`, refeito em `ReportApplicationService` para não acoplar um application service a outro), ordenado do maior percentual de uso para o menor e destacando em vermelho no PDF os orçamentos estourados. `ReportController` (`/api/reports/budget-vs-actual?referenceMonth=`)
- Exportação de transações em CSV — ✅ **feito** (adiantado, sétimo tipo de relatório na tela "Relatórios") — extrato bruto de todas as transações do usuário (em todas as contas) num mês, incluindo transferências, pronto para abrir no Excel/Google Sheets. Tem a coluna "Situação" (Paga/Pendente), também no PDF. `ReportController` (`/api/reports/transaction-export?referenceMonth=`) e `ReportApplicationService.generateTransactionExport` resolvem nome de conta/categoria/cliente
- Todos os relatórios em PDF **e** CSV, com filtro de formato — ✅ **feito**: os sete tipos de relatório agora aceitam `&format=PDF|CSV` no mesmo endpoint (`ReportFormat`, default `PDF`, exceto o extrato de transações cujo default é `CSV`). `ReportApplicationService` separou o cálculo de cada relatório (`buildXxxData`) da renderização, despachando para `ReceiptGeneratorPort` (PDF, `OpenPdfReceiptGenerator`) ou `ReportCsvExporterPort` (CSV, `ReportCsvExporter` — monta o CSV na mão, mesma filosofia do `CsvTransactionParser` mas na direção inversa, com `CsvWriter` compartilhado cuidando do BOM UTF-8, delimitador `;` e escaping). Frontend: um único seletor "Formato do arquivo" na tela "Relatórios", ao lado do "Tipo de relatório", vale para qualquer relatório escolhido
- Multi-moeda
...
- Exportação de relatórios (PDF/Excel) — ✅ **feito**: todos os sete tipos de relatório disponíveis em PDF e em CSV (abre no Excel/Google Sheets), com um filtro de formato só na tela; não existe geração de .xlsx binário — decisão consciente para não adicionar uma lib pesada (Apache POI) só para isso
...
- **NotificationPreferences** ✅: user_id (PK), contas_ligado, dias_antecedência, orçamentos_ligado, das_ligado — preferências dos alertas por e-mail
- **SentAlert** ✅: id, user_id, tipo (conta/orçamento 80%/orçamento 100%/DAS), chave de referência, enviado_em — evita repetir o mesmo aviso
