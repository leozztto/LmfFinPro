...
public class BudgetController {
    ...
    public List<BudgetResponse> list(@AuthenticationPrincipal AuthenticatedUser currentUser) {
        return budgetApplicationService.list(currentUser.userId()).stream().map(this::toResponse).toList();
    }

    @PostMapping
    public ResponseEntity<BudgetResponse> create(
        @AuthenticationPrincipal AuthenticatedUser currentUser, @Valid @RequestBody BudgetRequest request
    ) {
        Budget created = budgetApplicationService.create(
            currentUser.userId(), request.categoryId(), request.referenceMonth(), request.limitValue()
        );
