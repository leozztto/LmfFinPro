...
public class ClientController {
    ...
    public List<ClientResponse> list(@AuthenticationPrincipal AuthenticatedUser currentUser) {
        return clientApplicationService.list(currentUser.userId()).stream().map(mapper::toResponse).toList();
    }

    @GetMapping("/{id}")
    public ClientResponse getById(@AuthenticationPrincipal AuthenticatedUser currentUser, @PathVariable Long id) {
        return mapper.toResponse(clientApplicationService.getById(currentUser.userId(), id));
    }

    @PostMapping
    public ResponseEntity<ClientResponse> create(
        @AuthenticationPrincipal AuthenticatedUser currentUser, @Valid @RequestBody ClientRequest request
    ) {
        Client created = clientApplicationService.create(
            currentUser.userId(), request.name(), request.email(), request.phone(),
            request.documentType(), request.documentNumber(), request.workType(),
            request.notes(), request.color(), request.active()
        );
    ...
    public ClientResponse update(
        ...
        Client updated = clientApplicationService.update(
            currentUser.userId(), id, request.name(), request.email(), request.phone(),
            request.documentType(), request.documentNumber(), request.workType(),
            request.notes(), request.color(), request.active()
        );
