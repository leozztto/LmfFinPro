...
public class CategoryController {
    ...
    public List<CategoryResponse> list(@AuthenticationPrincipal AuthenticatedUser currentUser) {
        return categoryApplicationService.list(currentUser.userId()).stream().map(mapper::toResponse).toList();
    }

    @GetMapping("/{id}")
    public CategoryResponse getById(@AuthenticationPrincipal AuthenticatedUser currentUser, @PathVariable Long id) {
        return mapper.toResponse(categoryApplicationService.getById(currentUser.userId(), id));
    }

    @PostMapping
    public ResponseEntity<CategoryResponse> create(
        @AuthenticationPrincipal AuthenticatedUser currentUser, @Valid @RequestBody CategoryRequest request
    ) {
        Category created = categoryApplicationService.create(
            currentUser.userId(), request.name(), request.type(), request.color(), request.icon()
        );
