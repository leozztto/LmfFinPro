...
public class BudgetApplicationService {
    ...
    public Budget create(Long currentUserId, Long categoryId, YearMonth referenceMonth, BigDecimal limitValue) {
        return budgetRepositoryPort.save(Budget.create(currentUserId, categoryId, referenceMonth, limitValue));
    }
     ...
     * ficar dessincronizado (mesma convenção de {@code AccountApplicationService.calculateCurrentBalance}).
     */
    public BigDecimal calculateSpent(Budget budget) {
        return transactionRepositoryPort.sumAmountByUserIdAndCategoryIdAndTypeBetween(
            budget.userId(), budget.categoryId(), CategoryType.EXPENSE,
            budget.referenceMonth().atDay(1), budget.referenceMonth().plusMonths(1).atDay(1)
        );
    }
    ...
    private Budget findOwnedOrThrow(Long currentUserId, Long budgetId) {
        return budgetRepositoryPort.findById(budgetId)
            ...
            .orElseThrow(() -> new ResourceNotFoundException("Orçamento não encontrado: " + budgetId));
    }
}
