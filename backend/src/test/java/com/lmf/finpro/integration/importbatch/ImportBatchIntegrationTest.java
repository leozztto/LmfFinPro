...
class ImportBatchIntegrationTest extends AbstractIntegrationTest {
    ...
    void uploadsCsvAndCategorizesTransactionsUsingExistingRule() {
        ...
        // As duas transações do Uber batem com a regra pré-cadastrada; só a do cliente fica sem categoria.
        assertThat(batch.uncategorizedCount()).isEqualTo(1);

        ResponseEntity<TransactionResponse[]> transactionsResponse = restTemplate.exchange(
            "/api/import-batches/" + batch.id() + "/transactions", HttpMethod.GET,
            new HttpEntity<>(user.authHeaders()), TransactionResponse[].class
        );
        ...
        assertThat(transactions).filteredOn(t -> t.description().contains("UBER"))
    ...
    void reviewingImportedTransactionReinforcesCategoryRuleForFutureImports() {
        ...
        TransactionResponse[] firstTransactions = restTemplate.exchange(
            "/api/import-batches/" + firstBatch.id() + "/transactions", HttpMethod.GET,
            new HttpEntity<>(user.authHeaders()), TransactionResponse[].class
        ).getBody();
        TransactionResponse clientPayment = List.of(firstTransactions).stream()
        ...
        ResponseEntity<TransactionResponse> reviewResponse = restTemplate.exchange(
            "/api/import-batches/" + firstBatch.id() + "/transactions/" + clientPayment.id(), HttpMethod.PUT,
            new HttpEntity<>(review, user.authHeaders()), TransactionResponse.class
        );
        ...
        ResponseEntity<CategoryRuleResponse[]> rulesResponse = restTemplate.exchange(
            "/api/category-rules", HttpMethod.GET, new HttpEntity<>(user.authHeaders()), CategoryRuleResponse[].class
        );
        assertThat(rulesResponse.getBody()).anyMatch(rule ->
            rule.pattern().contains("ACME") && incomeCategoryId.equals(rule.categoryId())
        );
    ...
    void rejectsCsvWithInvalidHeader() {
        ...
        ResponseEntity<ApiError> response = restTemplate.exchange(
            "/api/import-batches", HttpMethod.POST,
            new HttpEntity<>(multipartBody(accountId, "coluna1,coluna2\nx,y\n"), multipartHeaders(user)), ApiError.class
        );
    ...
    void userCannotUploadToAnotherUsersAccount() {
        ...
        ResponseEntity<ApiError> response = restTemplate.exchange(
            "/api/import-batches", HttpMethod.POST,
            new HttpEntity<>(multipartBody(ownerAccountId, VALID_CSV), multipartHeaders(intruder)), ApiError.class
        );

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NOT_FOUND);
    }

    private ResponseEntity<ImportBatchResponse> uploadCsv(TestUser user, Long accountId, String csv) {
        return restTemplate.exchange(
            "/api/import-batches", HttpMethod.POST,
            new HttpEntity<>(multipartBody(accountId, csv), multipartHeaders(user)), ImportBatchResponse.class
        );
    }
    ...
    private void createRule(TestUser user, String pattern, Long categoryId) {
        ...
        restTemplate.exchange(
            "/api/category-rules", HttpMethod.POST,
            new HttpEntity<>(new CategoryRuleRequest(pattern, categoryId), user.authHeaders()), CategoryRuleResponse.class
        );
    }
    ...
    private Long createAccount(TestUser user) {
        AccountRequest accountRequest = new AccountRequest("Conta para importação", AccountType.CHECKING, BigDecimal.ZERO);
